...
<sub>_This post is a transcript with some tweaks (to tail content to a text version) from the talk I presented for Codeminer's brown bag session in April 7th, 2023._</sub>

Pattern Matching is a feature introduced by Ruby 2.7 that allows deep matching of structured values. It'll check the structure of a given value, it might be a String, Number or even a Class and bind the matched parts into local variables.

Basically, there are two different things we can do with pattern matching in Ruby. We can test values against some pattern as well as extract local variables from it.

# How we can do that?

We’re gonna find two different ways of using pattern matching in Ruby.

We can use the keyword “in” to match some pattern “in” a given expression and receive a true/false value as the return of this action.

The second way is using the operator “equals/greater than” that we used to call the “hash rocket” operator in the old times of Ruby 1.9, the difference between the “in” keyword and this operator is basically that you’ll receive a success or, if the pattern doesn’t match, an Error will be raised.
...
<sub>_It is important to say that the "hash rocket" version does not exist in version 2.7 and instead, the "in" pattern, behaves exactly like this operator behaves in Ruby 3, raising an exception._</sub>

You guys can assume that from here on, in every example I’ll show, you could use both the "in” or the “equals/greater than” operator.
...
We already had contact with a couple of them in our previous examples but, let's elaborate better about each one.
...
The array pattern is a little bit different because we're gonna use this pattern to match the items of a given array. By default we can only match the entire array unless we're using the _rest_ operator as we're gonna see in our examples.
...
The Hash pattern is one of the cases I mentioned before that we already had contact in our first examples. It matches symbol keys against a given pattern, this pattern may be only the existing key or we can also match against `key: value` or even `key: type`. We have a main difference when comparing to the Array pattern because now we can perform partial matching.
...
It is important to mention the `**nil` present in our last example above because using it, we can affirm that we only accept the pattern we're giving and no partial matching is accepted. In our example our hash must contains only a key called _id_ with an Integer value, that's the reason our matching fails.

#### Alternative Pattern

The alternative pattern is also very interesting. It is pretty simple and tells us that we can mix different patterns to compose a union of patterns against our value.
In our example below, we’re explicitly saying that we accept either ‘john’ or ‘mary’ as acceptable values. The same for the other examples where we’re accepting either an Integer or String as acceptable values for the _created_at_ key.
...
Last but not least. We can use pattern matching to bind matched values in local variables. In our examples below we can see that we're extracting the first item of the array in a variable called _head_ and the rest in another variable called, no surprise here, _rest_.
The example using a Hash is equally very interesting, we can see that we are able to use pattern matching to extract local variables from complex and nested structures. We are getting the first item of the collection of friends and assigning it to a local variable called _best_friend_.
